...
namespace Test02 {
    internal class Program {
        ...
        private static void Exercise04(int[] numbers) {
            ...
            var Numbers = numbers.Where(n => n >= 10 && n <= 50).ToList();

            Console.WriteLine("10以上50以下の数字:");
        ...
        private static void Exercise05(List<string> cities) {
            int count = cities.Count(city => city.Contains('n'));
        ...
        private static void Exercise07(List<string> cities) {
            var SortedCities = cities.OrderBy(city => city);
        ...
        private static void Exercise09(List<string> cities) {
            var sortedByLength = cities.OrderBy(city => city.Length).ThenBy(city => city);

            foreach (var city in sortedByLength) {
                Console.WriteLine($"{city} : {city.Length}文字");
            }
        }
        ...
        private static void Exercise10(List<string> cities) {
            var Sixcities = cities.Where(city => city.Length == 6);

            foreach (var city in Sixcities) {
                Console.WriteLine(city);
            }
